...
class NodeImplTest {
    ...
    fun testStart() {
        val node = newNodeBuilder(NodeId("A"), NodeEndpoint("A", "localhost", 2333))
            ?.build() as NodeImpl
//        timeout = ElectionTimeout(ScheduledFuture)
        node.role = FollowerNodeRole(1, null, null, ElectionTimeout.NONE)
        node.start()
...
//        val rpc: RequestVoteRpc = mockConnector.rpc as RequestVoteRpc
...
//        val mockConnector = node.context.connector as MockConnector
//        val result: RequestVoteResult = mockConnector.result as RequestVoteResult
...
//        Assert.assertEquals(1, role.term)
    ...
    fun testReplicateLog() {
        ...
        val mockConnector = node.context.connector as MockConnector
        Assert.assertEquals(0, mockConnector.messageCount)
...
//        val rpc: AppendEntriesRpc = messages[2].rpc as AppendEntriesRpc
//        Assert.assertEquals(1, rpc.term)
    }
...
//        val result: AppendEntriesResult? = connector.result as AppendEntriesResult?
//        if (result != null) {
//            Assert.assertEquals(1, result.term)
//        }
//        if (result != null) {
